...
from crypto import app,socketio
...
class Indicator:
    ...
    def __fetch_trades(self, exchange):
        logger.info(f"Fetching last trades for pair: {self.pair}")
        try:
            trades = getattr(ccxt, exchange)().fetch_trades(symbol=self.pair, limit=1)
            if trades:
                last_trade = trades[0]  # Get the first (last) trade from the list
                trade_data = {
                ...
                with app.app_context():
                    socketio.emit('last_trade', json.dumps(trade_data))
    ...
    def update_data(self):
        try:
            for exchange in self.exchanges:
                self.ohlcv = self.__fetch_trades(exchange)
            return self.ohlcv
...
def update_symbol_data(symbol,exchanges):
    ...
    try:
        stoch_data = indicator.update_data()
        '''for exchange in exchanges:
            file_name_stoch = f"trades/{symbol.replace('/', '').replace('.D', '', 1).replace('.T', '', 1)}_{exchange}.json"
            with open(file_name_stoch, 'w') as f:
                json.dump(stoch_data.to_dict(orient='records'), f)'''
